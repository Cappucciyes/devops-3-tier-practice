...
> **AWS 예약 IP**: 서브넷의 첫 4개와 마지막 1개. 예: `172.16.0.0/27` 은 `.0`, `.1`, `.2`, `.3`, `.31` 사용 불가 → 실제 할당 가능 `.4 ~ .30`
...
| **Dedicated Host** | 특정 물리 서버 자체를 지정해 사용 (라이선스 관리 등) |

VPC 생성 시 Tenancy를 `Dedicated`로 지정하면, **그 VPC에서 띄우는 모든 인스턴스가 전용 하드웨어로 강제**됩니다.

**왜 쓰는지**: 금융·의료·정부 등에서 "다른 회사와 물리 서버 공유 금지" 같은 컴플라이언스 요구가 있을 때 사용.

> ⚠️ **이번 가이드에서는 `Default` 그대로 두세요.**
> Dedicated는 시간당 비용이 크게 상승하고 (전용 하드웨어 요금이 별도 부과됨), 일반적인 서비스에는 불필요합니다. 컴플라이언스 요구가 명확하지 않은 한 절대 체크하지 마세요. **VPC 생성 후에는 Default로 되돌릴 수 없습니다.**
...
| `rt-private-a` | pri-elb-a, pri-svc-a, pri-db-a, pri-mgmt-a | **NAT GW** (`nat-a`) |
| `rt-private-c` | pri-elb-c, pri-svc-c, pri-db-c, pri-mgmt-c | **NAT GW** (`nat-c`) |
...
## 8. VPC Endpoint (S3) 생성

S3 트래픽을 인터넷/NAT을 거치지 않고 AWS 백본으로 전달 → **NAT 비용 절감 + 보안 향상**.

**경로**: VPC 콘솔 → **Endpoints** → **Create endpoint**
...
| VPC | `my-vpc` |
| Route tables | `rt-private-a`, `rt-private-c` 체크 |
| Policy | Full access (필요 시 좁히기) |

> Gateway Endpoint는 **무료**. 선택한 라우팅 테이블에 자동으로 S3 prefix list 라우트가 추가됩니다.
...
- [ ] Public 서브넷 6개에 auto-assign public IPv4 활성화
- [ ] IGW `my-igw` 가 `my-vpc` 에 Attached
- [ ] NAT GW 2개 Available, 각각 EIP 할당
- [ ] 라우팅 테이블 3개 생성 및 서브넷 연결 완료
- [ ] S3 Gateway Endpoint가 두 Private 라우팅 테이블에 연결됨

### 9-2. 통신 테스트 (선택)

| 위치 | 기대 동작 |
|---|---|
| `pub-svc-a`의 Bastion EC2 | 관리자 IP에서 SSH 접속 가능, 외부 인터넷 통신 가능 |
| `pri-svc-a`의 EC2 | 외부 인바운드 차단, 아웃바운드 인터넷은 NAT 통해 가능 |
| Bastion → `pri-svc-a` EC2 | SSH 점프 가능 |
